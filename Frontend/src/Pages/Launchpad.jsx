...
const Launchpad = () => {
    ...
    async (page = 1) => {
      ...
      try {
        const queryParam = searchQuery
          ? `&company=${encodeURIComponent(searchQuery)}`
          : "";
        const response = await fetch(
          `http://localhost:5000/api/jobs?page=${page}&limit=10&type=${activeTab}${queryParam}`
        );
  ...
  const sendFiltersToBackend = async () => {
    ...
    try {
      ...
      if (result.success) {
        setOpportunities(result.jobs); // Update UI with filtered jobs
      } else {
        setOpportunities([]); // No jobs found
      }
    } catch (error) {
      console.error("There was an error with the request:", error);
      setOpportunities([]); // Clear jobs on error
    }
  };

  // Handle applying filters
  const handleApplyFilters = () => {
    sendFiltersToBackend(); // Send the filters to the backend when the user applies them
                ...
                <div className="flex justify-end mt-4">
                  <button
                    onClick={async () => {
                      ...
                      sendFiltersToBackend(); // Send the filter data to the backend
                    }}
