...
const Dashboard = () => {
  ...
  const fetchAPI = useCallback(async (page = 1) => {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}appliedJobs/allDetails?page=${page}&search=${encodeURIComponent(searchQuery)}`, {
        method: 'GET',
        headers: {
  ...
  const filteredOpportunities = (opportunities || []).filter((opportunity) => {
    const lowerSearchQuery = searchQuery.trim().toLowerCase(); // Normalize search query
    const companyName = opportunity.jobDetails?.company?.toLowerCase() || ""; // Normalize company name

    return (
      (activeTab === "all" || opportunity.jobDetails.type === activeTab) &&
      companyName.includes(lowerSearchQuery) // Check if search query matches company
    );
  });

  const pageSize = 10;
  const dynamicTotalPages = Math.ceil(filteredOpportunities.length / pageSize);

  // Slice filtered opportunities for the current page
